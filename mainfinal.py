...
class ContactBookApp:
    ...
    def search_contact(self):
        ...
        if not query:
            self.update_display()
            return
        filtered_contacts = {name: number for name, number in self.contacts.items()
                            if query in name.lower() or query in number}
        if not filtered_contacts:
            messagebox.showerror("No encontrado", "No existe ningún contacto que coincida con la búsqueda.")
            self.update_display({})
            return
